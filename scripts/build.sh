...
case "$LAMBDA_os" in
    ...
    windows)
        ...
        $LAMBDA_cpp_compiler LDFLAGS=-L/a/_temp/msys64/mingw64/include `pkg-config --cflags gtk+-3.0` -o build SudokuSquare.cpp createSudokuPuzzle.cpp test.cpp `pkg-config --libs gtk+-3.0`
        ;;
    *)
        echo "$USAGE"
        exit 1
